...
__global__ void moeA2ACombineKernel(
    int const* send_indices,                   // [local_num_tokens, ep_size]
    ...
    int ep_size)
{
    ...
    int global_warp_id = blockIdx.x * WARPS_PER_BLOCK + warp_id_in_block;
    int local_token_idx = global_warp_id;
    const int size_per_token = elements_per_token * sizeof(T);
    ...
    for (int target_rank = 0; target_rank < ep_size; target_rank++)
    {
        ...
        if (dst_idx < 0) continue;  // Wasn't sent to this rank
        ...
        uint8_t const* recv_ptr = recv_buffer + 
            (rank_id * max_tokens_per_rank + dst_idx) * size_per_token;
        ...
        if(lane_id == 0)
        {
            ...
            float vals[5];
            for (int i = 0; i < 5; ++i) {
                uint16_t bf16_val = *(reinterpret_cast<const uint16_t*>(recv_ptr) + i);
                vals[i] = __bfloat162float(*reinterpret_cast<const __nv_bfloat16*>(&bf16_val));
            }
            int send_indices_vals[4];
            for (int i = 0; i < 4; ++i) {
                send_indices_vals[i] = send_indices[local_token_idx * ep_size + i];
            }
            printf("Rank %d token %d retrieved from rank %d, recv_ptr[0..4]=[%f, %f, %f, %f, %f] send_indices[0...3]=[%d, %d, %d, %d]\n",
                   rank_id, local_token_idx, target_rank, vals[0], vals[1], vals[2], vals[3], vals[4], send_indices_vals[0], send_indices_vals[1], send_indices_vals[2], send_indices_vals[3]);
        }
    ...
    //     // Only the first thread in the block will do the synchronization - exactly the same thread as flag initialization
    //     if(blockIdx.x == 0 && threadIdx.x == 0) {
...
void moe_a2a_combine_launch(MoeA2ACombineParams const& params)
{
    ...
    case nvinfer1::DataType::kHALF:
        moeA2ACombineKernel<half><<<grid_size, kBlockSize, 0, params.stream>>>(
            params.send_indices, kernel_ptrs,
            params.local_token_counter,
    ...
    case nvinfer1::DataType::kBF16:
        moeA2ACombineKernel<__nv_bfloat16><<<grid_size, kBlockSize, 0, params.stream>>>(
            params.send_indices, kernel_ptrs,
            params.local_token_counter,
            params.max_tokens_per_rank, params.elements_per_token,
            params.local_num_tokens, params.ep_rank, params.ep_size);
    ...
    case nvinfer1::DataType::kFLOAT:
        moeA2ACombineKernel<float><<<grid_size, kBlockSize, 0, params.stream>>>(
            params.send_indices, kernel_ptrs,
            params.local_token_counter,
            params.max_tokens_per_rank, params.elements_per_token,
            params.local_num_tokens, params.ep_rank, params.ep_size);
    ...
    default:
        TLLM_CHECK_WITH_INFO(false, "Unsupported data type for moe_a2a_combine");
    }
}
