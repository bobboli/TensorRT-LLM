...
from tensorrt_llm._torch.distributed.ops import moe_a2a_dispatch, moe_a2a_combine
...
def fake_moe(hidden_states, token_selected_experts, token_final_scales, rank, ep_size):
    """
...
def make_bfloat16_payloads(local_num_tokens: int, hidden_size: int, top_k: int,
    ...
    hidden_states = torch.randn(local_num_tokens, hidden_size,
                                dtype=torch.bfloat16,
                                device='cuda')
...
class TestMoEAlltoAll:
    ...
    @pytest.mark.parametrize("mpi_pool_executor,all_num_tokens,top_k,dtype", [
    ...
    def test_combine(self, mpi_pool_executor, all_num_tokens, top_k, dtype):
        ...
        ep_size = mpi_pool_executor.num_workers
        assert ep_size == len(all_num_tokens), "ep_size does not match all_num_tokens"
        assert torch.cuda.device_count() >= ep_size, f"Need at least {ep_size} GPUs"
        ...
        results = mpi_pool_executor.map(
            ...
            *zip(*[(ep_size, all_num_tokens, top_k, workspace_size_per_rank,
                    num_experts_per_rank, hidden_size, max_tokens_per_rank, dtype)] * ep_size),
        )
        ...
        try:
            all_results = list(results)
            print(f"Successfully collected results from {len(all_results)} workers")
        ...
        verify_combine_results(all_results, ep_size, all_num_tokens, top_k, hidden_size)


def run_moe_a2a_dispatch_and_combine_single_rank(ep_size, all_num_tokens, top_k,
                                                 workspace_size_per_rank, num_experts_per_rank,
                                                 hidden_size, max_tokens_per_rank, dtype):
    ...
    try:
        mapping = Mapping(rank=rank, tp_size=ep_size, pp_size=1, cp_size=1, world_size=ep_size)
        ...
        payloads = make_bfloat16_payloads(
            rank_local_tokens, hidden_size, top_k, rank, token_selected_experts)
        ...
        hidden_states_recv = recv_buffers[0] # [ep_size, max_tokens_per_rank, hidden_size]
        token_selected_experts_recv = recv_buffers[1] # [ep_size, max_tokens_per_rank, top_k]
        token_final_scales_recv = recv_buffers[2] # [ep_size, max_tokens_per_rank, top_k]
        ...
        hidden_states_recv_cloned = torch.empty_like(hidden_states_recv, device=torch.cuda.current_device())
        ...
        workspace[rank, combine_flags_offset:combine_flags_offset + ep_size * 4].zero_()
        ...
        combined_output = moe_a2a_combine(
            send_indices,
            hidden_states_recv_cloned,
            workspace,
            max_tokens_per_rank,
            rank,
            ep_size,
        ...
        # Each rank has ep_size flags (one from each source rank). Check all the flags are marked.
        completion_flags_ptr = workspace[rank, completion_flags_offset:completion_flags_offset + ep_size * 4].clone()
        ...
        if not torch.all(completion_flags == expected_flags).item():
            print(f"ERROR: Rank {rank} completion flags after combine: {completion_flags.tolist()}, expected: {expected_flags.tolist()}")
...
def verify_combine_results(all_results, ep_size, all_num_tokens, top_k, hidden_size):
    ...
    for rank in range(ep_size):
        ...
        for token_idx in range(local_num_tokens):
            ...
            torch.testing.assert_close(
                actual, expected_sum, rtol=1e-3, atol=1e-5,
                msg=(
                    f"Rank {rank}, Token {token_idx}: Mismatch!\n"
